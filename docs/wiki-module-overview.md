...
> **版本**: v2.1  
> **日期**: 2026-09-12  
> **状态**: 实施中（Stage 1 切片已落地：G7 知识库模型 + 分层 schema/repo/service + 管理后台路由）
...
| G3 | 分层架构违约（schemas/services 空，逻辑内联） | 高（可维护性） | 中 | **P2** | 部分落地（wiki.py 用户侧端点已委托 service/repo 层，响应契约不变） | §5 |
| G4 | 编辑页适配 hack（全量拉列表按 id 找 slug） | 中（性能/正确性） | 低 | **P2** | 部分落地（admin 新增按 id 取详情端点） | §5 |
| G5 | backlinks 只加不清 | 低 | 低 | **P3** | 待落地 | §5 |
| G6 | LLM 自动摘要/成稿缺失 | 中 | 高 | **P3** | 待评估 | — |
| G7 | 缺知识库实体 `wiki_knowledge` 与目录归属字段 | 高（模型与导航基础） | 中 | **P1** | 部分落地（模型+管理后台路由已建，前端未接） | §3/§5 |
| G8 | 内容展示未分级（知识库→目录→文章→版本） | 高（核心体验） | 中 | **P1** | 待落地 | §6 |
| G9 | LLM Wiki 检索体验未定义 | 高（差异化能力） | 中 | **P1** | 待落地 | §3 |
| G10 | 管理控制台未建设 | 中（运营效率） | 低 | **P2** | 部分落地（后端管理路由已建，前端 Tab 页未实现） | §6 |
