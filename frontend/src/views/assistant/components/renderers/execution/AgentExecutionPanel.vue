<!--
  AgentExecutionPanel.vue — 智能体模式执行详情面板（独立组件，不复用 SkillExecutionPanel）

  与 SkillExecutionPanel 展示结构对齐（思考过程 / 执行事件 / 时间线三个 Tab + 结果正文在下），
  实现完全独立，便于后续做智能体模式专属的个性化展示（工具调用链、ReAct 迭代可视化等）。

  与思考模式的差异：智能体的思考来源于模型原生思维链（THINKING_BLOCK_*），
  每个思考块聚合为一个推理步骤；工具调用链是智能体的核心要素，在「执行事件」Tab 完整展开。
-->
<template>
  <div class="agent-execution-panel">
    <div class="execution-events-header" @click="collapsed = !collapsed">
      ...
      <RobotOutlined class="execution-icon" />
      <span class="execution-label">智能体执行</span>
      <a-badge :count="allEvents.length" :number-style="{ backgroundColor: 'var(--accent)', fontSize: '10px' }" />
      <span v-if="streaming" class="exec-running-indicator"><LoadingOutlined spin /> 执行中...</span>
    </div>
        ...
        <a-tab-pane key="thinking">
          ...
          <div v-if="displayThinkingSteps.length" class="thinking-steps-list">
            <div v-for="s in displayThinkingSteps" :key="s.step" class="thinking-step-card">
              ...
              <div v-show="expandedSteps[s.step]" class="thinking-content">{{ s.content }}</div>
            </div>
          </div>
          <div v-else class="exec-empty">暂无思考记录</div>
        </a-tab-pane>
        <!-- Tab 2: 执行事件（合并「思考过程」+「执行事件」+「时间线步骤」；最终 result 由父级单独展示，不含在内） -->
        <a-tab-pane key="events">
          <template #tab>
            <ToolOutlined style="color: var(--accent-2)" />
            <span>执行事件</span>
          </template>
          <!-- 思考过程（来自「思考过程」Tab，不含最终 result） -->
          <div v-if="displayThinkingSteps.length" class="exec-section">
            <div class="exec-section-header"><BulbOutlined style="color: #722ed1" /><span>思考过程</span></div>
            <div class="thinking-steps">
              <div v-for="s in displayThinkingSteps" :key="s.step" class="thinking-step">
                <div class="thinking-step-header" @click="toggleStepExpand(s.step)">
                  <CaretRightOutlined :class="{ expanded: expandedSteps[s.step] }" />
                  <span class="thinking-step-title">{{ s.title }}</span>
                  <span v-if="s.time" class="thinking-step-time">{{ formatTime(s.time) }}</span>
                </div>
                <div v-show="expandedSteps[s.step]" class="thinking-content">{{ s.content }}</div>
              </div>
            </div>
          ...
          <!-- 时间线步骤（来自「时间线」Tab，不含最终 result） -->
          <div v-if="finalSteps.length" class="exec-section">
            <div class="exec-section-header"><HistoryOutlined style="color: var(--info)" /><span>时间线步骤</span></div>
            <TimelineFlowPlayer :steps="finalSteps" :artifacts="finalArtifacts" :live="live" :panel-id="panelId" :coalesce="true" :hide-empty="true" />
          </div>
          <div v-if="!eventsTabHasContent" class="exec-empty">
            暂无执行事件记录
          </div>
        </a-tab-pane>
        <!-- Tab 3: 时间线 -->
...
import {
  ...
  BulbOutlined, ToolOutlined, FileOutlined, HistoryOutlined, RobotOutlined,
...
/** 从 API 加载历史执行事件（与 skill / thinking 模式共用同一回放接口） */
...
/** 执行事件 Tab 是否有内容（思考/工具/步骤/产物/时间线任一非空；最终 result 不在其中，天然排除） */
const eventsTabHasContent = computed(() =>
  displayThinkingSteps.value.length || allToolEvents.value.length ||
  progressEvents.value.length || artifactEvents.value.length || finalSteps.value.length
)

/** 从扁平事件流按 step 聚合思考步骤（append 增量语义） */
...
.execution-events-header {
  ...
  .execution-icon { font-size: 13px; color: var(--accent); }
