  ...
  <div class="ext-inst">
    ...
    <a-modal v-model:open="formOpen" :title="form.id ? t('kbMgmt.common.edit') : t('kbMgmt.common.create')" @ok="save">
      <a-form :model="form" layout="vertical">
        ...
        <a-form-item :label="t('kbMgmt.common.type')">
        ...
        <a-form-item :label="t('kbMgmt.ext.syncEnabled')">
          <a-switch v-model:checked="form.sync_enabled" />
        </a-form-item>
        <a-form-item :label="t('kbMgmt.ext.targetDataset')" v-if="form.sync_enabled"><a-input v-model:value="form.kb_dataset" /></a-form-item>
        <a-form-item :label="t('kbMgmt.ext.syncInterval')" v-if="form.sync_enabled"><a-input-number v-model:value="form.sync_interval_min" :min="1" style="width:100%" /></a-form-item>
      </a-form>
    </a-modal>
  </div>
</template>
...
const columns = computed(() => [
  ...
  { title: t('kbMgmt.common.type'), dataIndex: 'connector_type', key: 'connector_type', width: 120 },
  { title: t('kbMgmt.ext.syncCol'), key: 'sync_enabled', width: 80 },
  { title: t('kbMgmt.ext.targetDataset'), dataIndex: 'kb_dataset', key: 'kb_dataset' },
  { title: t('kbMgmt.ext.lastSync'), dataIndex: 'last_sync_at', key: 'last_sync_at', width: 160 },
...
async function load() {
  ...
  try {
    ...
    message.warning(t('kbMgmt.ext.routerMissing'))
  } finally {
    loading.value = false
  }
}
...
async function save() {
  try {
    ...
    message.success(t('kbMgmt.ext.saveSuccess'))
    formOpen.value = false
    load()
  } catch (e: any) {
    message.error(e?.response?.data?.detail || t('kbMgmt.ext.saveFailed'))
  }
}
async function remove(row: any) {
  try {
    ...
    message.error(t('kbMgmt.ext.deleteFailed'))
  }
}
async function toggleSync(row: any, v: boolean) {
  try {
    await api.setSync(row.id, v)
    message.success(t('kbMgmt.ext.syncToggled'))
    load()
  } catch {
    message.warning(t('kbMgmt.ext.syncRouterMissing'))
  }
}
onMounted(load)
</script>
